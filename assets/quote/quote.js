...
/* ---------- hero: live 3D, then clean finished jobs you can swipe ---------- */
var MODES={
  r3d:{lbl:"Live 3D",note:"A real size roof. Tap to pick your home, roof, panels and spinners."},
  win:{lbl:"Windows",note:"Real finished jobs. Swipe for more."},
  sol:{lbl:"Solar",note:"Real finished jobs. Swipe for more."},
  pig:{lbl:"Pigeon proofing",note:"Real finished jobs. Swipe for more."}
};
var setMode=(function(){
  var box=$("wipe"), cur=null;
  [].forEach.call(box.querySelectorAll(".car"),function(c){
    var tr=c.querySelector(".track"), n=tr.children.length, dots=c.querySelector(".dots"), i;
    for(i=0;i<n;i++)dots.appendChild(document.createElement("i"));
    function mark(){var k=Math.round(tr.scrollLeft/Math.max(1,tr.clientWidth));[].forEach.call(dots.children,function(d,j){d.className=j===k?"on":"";});c._k=k;}
    tr.addEventListener("scroll",function(){mark();},{passive:true});
    ["pointerdown","touchstart","wheel"].forEach(function(t){tr.addEventListener(t,function(){c._held=Date.now();},{passive:true});});
  ...
  setInterval(function(){if(cur&&!document.hidden)cur._next();},3800);
  function set(m){
    if(!MODES[m])return;
    [].forEach.call($("modes").children,function(x){x.setAttribute("aria-pressed",String(x.dataset.m===m));});
    cur=null;[].forEach.call(box.querySelectorAll(".lay"),function(l){l.hidden=l.dataset.lay!==m;if(!l.hidden&&l.classList.contains("car"))cur=l;});
    $("wlbl").textContent=MODES[m].lbl; $("wnote").textContent=MODES[m].note; $("wpct").hidden=true; window.__heroMode=m;
...
tiles.addEventListener("click",function(e){
  ...
  if(st[k]&&MODES[k])setMode(k); if(st[k]&&window.__t3&&window.__t3.svc)window.__t3.svc(k); track("select_service",{service:k,on:st[k]}); render();
});
...
/* ---------- 3D: your home and the neighbors, live in the hero, full screen on tap ---------- */
(function(){
  var ov=$("ov"), loading=false, api=null, lastFocus=null, autoAfter=0, pend=[];
  var CF=window.TQ||{}, SVC=["pig","win","sol","scr"];
  var h3={stories:1,roof:0,color:0,after:0,stage:0,spin:3,cov:true,arrays:1,svc:(CF.svc&&CF.svc[0])||"pig"};
  var CYCLE=(CF.tab||"r3d")==="r3d"&&(!CF.svc||CF.svc[0]==="win"), cycleI=0;
  function free(){return st.panels>=16?3:2;}
  var TITLE={pig:"Pigeon proofing in 3D",win:"Window cleaning in 3D",sol:"Solar cleaning in 3D",scr:"Screen repair in 3D"};
  function syncUI(){segSync("h3v",SVC.indexOf(h3.svc));ov.setAttribute("data-svc",h3.svc);$("w3more").setAttribute("aria-checked",String(st.more));$("c3n").textContent=st.screens;$("ovT").textContent=TITLE[h3.svc];
    segSync("h3s",h3.stories);segSync("h3a",h3.arrays);segSync("h3r",h3.roof);segSync("h3c",h3.color);segSync("tq-ba",h3.after);segSync("stg",h3.stage);
  ...
  /* the story plays itself once when the builder opens */
  function play(){clearTimeout(autoAfter);
    if(h3.svc==="pig"){h3.stage=0;syncUI();if(api)api.state();autoAfter=setTimeout(function(){h3.stage=1;syncUI();if(api)api.state();autoAfter=setTimeout(function(){h3.stage=2;syncUI();if(api)api.state();},2600);},1800);}
    else{h3.after=0;syncUI();if(api)api.state();autoAfter=setTimeout(function(){h3.after=1;syncUI();if(api)api.state();},1500);}}
  function open(){
    lastFocus=document.activeElement; h3.stories=st.stories; h3.spin=free()+st.spin; syncUI();
    ov.hidden=false; document.documentElement.style.overflow="hidden"; $("ovX").focus(); track("view_3d",{service:h3.svc});
    load(function(){api.attach($("stage"));api.rebuild();play();});
  }
  ...
  pick("h3v",function(v){h3.svc=SVC[v];if(api)api.aim();play();track("view_3d_service",{service:h3.svc});});
  $("w3more").addEventListener("click",function(){st.more=!st.more;if(!st.win)st.win=true;render();syncUI();if(api)api.rebuild();});
  $("c3m").addEventListener("click",function(){st.screens=Math.max(1,st.screens-1);if(!st.scr)st.scr=true;render();syncUI();if(api)api.state();});
  $("c3p").addEventListener("click",function(){st.screens=Math.min(40,st.screens+1);if(!st.scr)st.scr=true;render();syncUI();if(api)api.state();});
  ...
  pick("stg",function(v){clearTimeout(autoAfter);h3.stage=v;if(api)api.state();track("pigeon_stage",{stage:v});});
  ...
  function setSpin(n,look){clearTimeout(autoAfter);h3.spin=Math.max(0,Math.min(10,n));h3.stage=2;st.spin=Math.max(0,h3.spin-free());render();syncUI();if(api){api.state();if(look)api.lookBack();}}
  ...
  window.__t3={update:function(){if(api)api.panels();},svc:function(k){if(SVC.indexOf(k)>=0){h3.svc=k;CYCLE=false;if(api&&ov.hidden){api.state();api.aim();}}}};
  ...
  function init(){
    ...
    var root=new T.Group(); scene.add(root); var house=root;
    ...
    var M={
      ...
      glass:Std({color:0x1b2733,metalness:.9,roughness:.05}), glassDull:Std({color:0x4a5561,metalness:.3,roughness:.55}), shine:new T.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:0,depthWrite:false,blending:T.AdditiveBlending}), garage:Std({map:garageTex,roughness:.6}), concrete:Std({map:concreteTex,roughness:.95}),
      ...
      pframe:Std({color:0x1c1e21,metalness:.7,roughness:.35}), cell:Std({map:cellTex,metalness:.3,roughness:.1,envMapIntensity:1.4}), cellN:Std({map:cellTex,metalness:.3,roughness:.1,envMapIntensity:1.4}), road:Std({color:0x5a5d61,roughness:.95}), curb:Std({color:0xcfccc4,roughness:.9}), rail:Std({color:0xa3a9b0,metalness:.8,roughness:.4}),
      ...
      scrNew:new T.MeshStandardMaterial({map:tex(64,64,function(g,w,h){g.strokeStyle="rgba(30,32,36,.75)";g.lineWidth=1;for(var i=0;i<=w;i+=3){g.beginPath();g.moveTo(i,0);g.lineTo(i,h);g.stroke();g.beginPath();g.moveTo(0,i);g.lineTo(w,i);g.stroke();}}),transparent:true,roughness:.8,depthWrite:false}),
      sframe:Std({color:0x8e9399,metalness:.6,roughness:.4}),
    ...
    var H=.2, TOP=.07, HALF=1.6, me=null, NB=[], P={skirt:[],clips:[],birds:[],wins:[],cells:[],shine:[]}, built="", spinGroup=[], ventGroup=[], covC={}, covT={}, covM={};
    var target=new T.Vector3(), camDist=20, mt=0, ms=0, shineT=0;
    function box(w,h,d,m,x,y,z,par,noCast){var o=new T.Mesh(new T.BoxGeometry(w,h,d),m);o.position.set(x,y,z);o.castShadow=!noCast;o.receiveShadow=true;par.add(o);return o;}
    /* one house: walls, windows, tiled roof, solar arrays. Used for your home and the neighbors. */
    function makeHouse(ox,cfg,r){
      var g=new T.Group();g.position.x=ox;root.add(g);
      var PW=1.02,PD=1.72,tile=TILE[cfg.roof],TT=TOP+tile.h,n=cfg.n,A=Math.min(cfg.arrays,n);
      ...
      var Wd=Math.max(11,need+4.5), L=Math.max(5.4,maxRows*PD+2.6), a=.4, run=L*Math.cos(a), rise=L*Math.sin(a), Dp=2*run, wallH=cfg.stories*2.8, o=.45;
      var h={g:g,W:Wd,L:L,wallH:wallH,rise:rise,Dp:Dp,TT:TT,ox:ox,edges:[],rects:{1:[],"-1":[]}};
      box(Wd,wallH,Dp,M.wall,0,wallH/2,0,g);
      if(cfg.stories>1)box(Wd+.06,.12,Dp+.06,M.trim,0,2.8,0,g);
      function win(x,y,z,ang){var w=new T.Group();w.position.set(x,y,z);w.rotation.y=ang;g.add(w);
        box(1.5,1.28,.1,M.trim,0,0,.03,w); var gl=box(1.3,1.08,.06,cfg.main?M.glassDull:M.glass,0,0,.07,w,true); box(.05,1.08,.08,M.trim,0,0,.09,w,true); box(1.62,.08,.2,M.trim,0,-.68,.08,w);
        if(!cfg.main)return;
        var sh=new T.Mesh(G.plane,M.shine);sh.scale.set(.35,1.3,1);sh.rotation.z=.5;sh.position.z=.105;sh.visible=false;w.add(sh);P.shine.push({m:sh,w:1.3});
        var sf=new T.Group();sf.position.z=.13;box(1.34,.04,.03,M.sframe,0,.55,0,sf,true);box(1.34,.04,.03,M.sframe,0,-.55,0,sf,true);box(.04,1.1,.03,M.sframe,-.65,0,0,sf,true);box(.04,1.1,.03,M.sframe,.65,0,0,sf,true);
        var sn=new T.Mesh(G.plane,M.scrNew);sn.scale.set(1.28,1.06,1);sf.add(sn);sf.visible=false;w.add(sf);P.wins.push({gl:gl,sf:sf,sh:sh});}
      var fxs=cfg.more?[-.38,-.13,.13,.38]:[-.32,0,.32];
      for(var s=0;s<cfg.stories;s++){[1,-1].forEach(function(side){fxs.forEach(function(fx,k){
        var y=s*2.8+1.5,x=fx*Wd,z=side*(Dp/2);
        if(s===0&&side===1&&Math.abs(fx)<.2){if(k===Math.floor(fxs.length/2)){box(2.8,2.2,.08,M.garage,0,1.1,z+.03,g);box(3.05,.12,.12,M.trim,0,2.27,z+.04,g);}return;}
        win(x,y,z,side>0?0:Math.PI);});});
        [1,-1].forEach(function(sx){[-.22,.22].forEach(function(fz){win(sx*Wd/2,s*2.8+1.5,fz*Dp,sx>0?Math.PI/2:-Math.PI/2);});});}
      var gShape=new T.Shape(); gShape.moveTo(-run,0); gShape.lineTo(run,0); gShape.lineTo(0,rise); gShape.lineTo(-run,0);
      [1,-1].forEach(function(sx){var gm=new T.Mesh(new T.ShapeGeometry(gShape),M.wall);gm.material.side=T.DoubleSide;gm.rotation.y=Math.PI/2;gm.position.set(sx*Wd/2,wallH,0);gm.castShadow=true;gm.receiveShadow=true;g.add(gm);});
      /* yard */
      box(3.4,.03,8.6,M.concrete,0,.015,Dp/2+4.3,g,true); box(1.1,.03,3,M.concrete,Wd*.32,.015,Dp/2+1.5,g,true);
      for(var q=0;q<4;q++){var sb=new T.Mesh(G.ball,M.shrub);var sc=.35+r()*.35;sb.scale.set(sc,sc*.8,sc);sb.position.set((q%2?1:-1)*(2.6+r()*(Wd/2-2.4)),sc*.6,Dp/2+.8+r()*.8);sb.castShadow=true;g.add(sb);}
      /* roof faces with real tiles */
      var pal=PAL[cfg.color], col=new T.Color();
      function face(sign){var f=new T.Group();f.rotation.order="YXZ";f.rotation.set(a,sign>0?0:Math.PI,0);f.position.set(0,wallH+rise/2,sign*run/2);g.add(f);
        box(Wd+2*o,.06,L+o,M.under,0,TOP-.03,o/2,f);
        var x0=-(Wd/2+o)+tile.dx/2, x1=Wd/2+o-tile.dx/2, z0=-L/2+tile.dz/2, z1=L/2+o-tile.dz/2;
        var nx=Math.floor((x1-x0)/tile.dx)+2, nz=Math.floor((z1-z0)/tile.dz)+1, im=new T.InstancedMesh(tile.geo,cfg.roof===2?M.shingle:M.tile,nx*nz), dm=new T.Object3D(), cnt=0;
        for(var zi=0;zi<nz;zi++)for(var xi=0;xi<nx;xi++){var off=tile.stag?((zi*tile.stag)%1)*tile.dx:0,x=x0+xi*tile.dx-off+(tile.stag?tile.dx/2:0);
          if(x<x0-.01||x>x1+.01)continue; dm.position.set(x,TOP+(cfg.roof===0?0:tile.h/2),z0+zi*tile.dz); dm.rotation.set(tile.tilt,0,0); dm.updateMatrix(); im.setMatrixAt(cnt,dm.matrix);
          var p=cfg.roof===0?pal[Math.floor(r()*3)]:pal[r()<.8?0:Math.floor(r()*3)],v=cfg.roof===0?.9+r()*.18:.95+r()*.09; col.setRGB(p[0]*v,p[1]*v,p[2]*v).convertSRGBToLinear(); im.setColorAt(cnt,col); cnt++;}
        im.count=cnt; im.instanceMatrix.needsUpdate=true; if(im.instanceColor)im.instanceColor.needsUpdate=true; im.receiveShadow=true; f.add(im);
        box(Wd+2*o,.24,.05,M.trim,0,TOP-.12,L/2+o,f); [1,-1].forEach(function(sx){box(.05,.24,L+o,M.trim,sx*(Wd/2+o),TOP-.12,o/2,f);});
        f.userData.sign=sign; return f;}
      h.F=face(1); h.B=face(-1);
      col.setRGB(pal[0][0]*.85,pal[0][1]*.85,pal[0][2]*.85).convertSRGBToLinear();
      var rg=new T.Mesh(new T.CylinderGeometry(.18,.18,Wd+2*o,16),Std({color:col.clone(),roughness:.75})); rg.rotation.z=Math.PI/2; rg.position.set(0,wallH+rise+TOP+.05,0); rg.castShadow=true; g.add(rg);
      /* solar arrays */
      var zTop=-L/2+1.35;
      groups.forEach(function(gp){
        var par=gp.face>0?h.F:h.B, ox2=gp.cx-(gp.cols-1)/2*PW, occ={}, py=TT+H;
        for(var i=0;i<gp.n;i++)occ[(i%gp.cols)+","+Math.floor(i/gp.cols)]=1;
        for(var rw=0;rw<gp.rows;rw++){var inRow=Math.min(gp.cols,gp.n-rw*gp.cols),rz=zTop+rw*PD+PD/2;[-.55,.55].forEach(function(dz){box(inRow*PW,.04,.05,M.rail,ox2+(inRow-1)*PW/2,TT+H/2,rz+dz,par);});}
        for(i=0;i<gp.n;i++){var c=i%gp.cols,rw2=Math.floor(i/gp.cols),x=ox2+c*PW,z=zTop+rw2*PD+PD/2;
          var fr=new T.Mesh(G.frame,M.pframe);fr.position.set(x,py,z);fr.castShadow=true;par.add(fr);
          var ce=new T.Mesh(G.cell,cfg.main?M.cell:M.cellN);ce.position.set(x,py+.024,z);par.add(ce);
          if(cfg.main){var sh=new T.Mesh(G.plane,M.shine);sh.rotation.x=-Math.PI/2;sh.rotation.z=.6;sh.scale.set(.3,1.8,1);sh.position.set(x,py+.03,z);sh.visible=false;par.add(sh);P.shine.push({m:sh,w:1,panel:1});}
          if(!occ[c+","+(rw2-1)])h.edges.push([x,z-PD/2,0,-1,gp.face]); if(!occ[c+","+(rw2+1)])h.edges.push([x,z+PD/2,0,1,gp.face]);
          if(!occ[(c-1)+","+rw2])h.edges.push([x-PW/2,z,1,-1,gp.face]); if(!occ[(c+1)+","+rw2])h.edges.push([x+PW/2,z,1,1,gp.face]);}
        h.rects[gp.face].push([ox2-PW/2-.4,ox2+(gp.cols-.5)*PW+.4,zTop-.4,zTop+gp.rows*PD+.4]);
      });
      return h;
    }
    /* open roof spots on a house, away from the panels */
    function roofSpots(h,fc,count,r){var out=[],tries=0,par=fc>0?h.F:h.B;while(out.length<count&&tries<500){tries++;var x=(r()-.5)*(h.W-1.2),z=-h.L/2+.5+r()*(h.L-1),hit=false;
      h.rects[fc].forEach(function(q){if(x>q[0]&&x<q[1]&&z>q[2]&&z<q[3])hit=true;}); if(!hit)out.push({par:par,face:fc,x:x,z:z,y:h.TT});}return out;}
    function world(par,x,y,z){return par.localToWorld(new T.Vector3(x,y,z));}
    function build(){
      while(root.children.length)root.remove(root.children[0]);
      P={skirt:[],clips:[],birds:[],wins:[],cells:[],shine:[]}; covM={}; spinGroup=[]; ventGroup=[]; NB=[];
      var r=rng(st.panels*97+13+h3.stories*7+h3.arrays*3);
      me=makeHouse(0,{n:st.panels,arrays:h3.arrays,roof:h3.roof,color:h3.color,stories:h3.stories,more:st.more,main:true},r);
      /* the neighbors: same street, their own roofs, nobody has protected them */
      var cfgs=[{n:12,arrays:1,roof:(h3.roof+1)%3,color:(h3.color+1)%4,stories:1},{n:14,arrays:1,roof:h3.roof,color:(h3.color+3)%4,stories:2}];
      var gap=5.5, lx=-(me.W/2+gap), rx=me.W/2+gap;
      var nL=makeHouse(0,cfgs[0],r); nL.g.position.x=lx-nL.W/2; nL.ox=nL.g.position.x;
      var nR=makeHouse(0,cfgs[1],r); nR.g.position.x=rx+nR.W/2; nR.ox=nR.g.position.x;
      NB=[nL,nR];
      /* street: asphalt, curb and sidewalk in front of every house */
      var Dmax=Math.max(me.Dp,nL.Dp,nR.Dp), zr=Dmax/2+9.6;
      box(260,.02,7,M.road,0,.01,zr,root,true); box(260,.08,.25,M.curb,0,.04,zr-3.6,root,true); box(260,.03,1.6,M.concrete,0,.015,zr-4.6,root,true);
      [me,nL,nR].forEach(function(hh){box(3.4,.03,zr-3.7-hh.Dp/2-8.6>0?zr-3.7-hh.Dp/2-8.6:.01,M.concrete,hh.ox,.015,(hh.Dp/2+8.6+zr-3.7)/2,root,true);});
      /* your mesh and clips */
      me.edges.forEach(function(e){var par=e[4]>0?me.F:me.B,len=e[2]?1.72:1.02,sk=new T.Mesh(G.plane,M.mesh);sk.position.set(e[0],me.TT+H/2,e[1]);if(e[2])sk.rotation.y=Math.PI/2;sk.scale.set(len,H,1);sk.userData.y0=me.TT;par.add(sk);P.skirt.push(sk);
        for(var qq=-1;qq<=1;qq+=2){var cl=new T.Mesh(G.clip,M.clip);cl.position.set(e[0]+(e[2]?0:qq*len*.3),me.TT+H-.01,e[1]+(e[2]?qq*len*.3:0));if(e[2])cl.rotation.y=Math.PI/2;par.add(cl);P.clips.push(cl);}});
      /* pigeons: yours (under panels, open roof, ridge) and the neighbors' */
      root.updateMatrixWorld(true);
      var landing=[];
      NB.forEach(function(nh){roofSpots(nh,1,10,r).concat(roofSpots(nh,-1,8,r)).forEach(function(s){landing.push(world(s.par,s.x,s.y,s.z));});});
      landing.sort(function(){return r()-.5;});
      function bird(pos,kind,info){var pg=pigeon();pg.position.copy(pos);pg.rotation.y=r()*6.28;root.add(pg);
        var b={m:pg,kind:kind,face:info.face,x:info.x,z:info.z,base:pos.clone(),k:0,kt:0,ry:pg.rotation.y};
        if(kind!=="N"){b.dest=landing.length?landing.pop():pos.clone().add(new T.Vector3(r()<.5?-14:14,0,0));} P.birds.push(b);}
      me.edges.slice().sort(function(){return r()-.5;}).slice(0,5).forEach(function(e){var par=e[4]>0?me.F:me.B,x=e[0]+(e[2]?.32*e[3]:(r()-.5)*.4),z=e[1]+(e[2]?(r()-.5)*.6:.32*e[3]);bird(world(par,x,me.TT,z),"U",{face:e[4],x:x,z:z});});
      roofSpots(me,1,6,r).concat(roofSpots(me,-1,8,r)).forEach(function(s){bird(world(s.par,s.x,s.y,s.z),"O",s);});
      for(var q=0;q<3;q++){var x=(r()-.5)*(me.W-1.5);bird(new T.Vector3(x,me.wallH+me.rise+TOP+.2,0),"R",{face:0,x:x,z:0});}
      NB.forEach(function(nh){roofSpots(nh,1,4,r).concat(roofSpots(nh,-1,3,r)).forEach(function(s){bird(world(s.par,s.x,s.y,s.z),"N",s);});
        var nr=nh.g.position.x; for(var k=0;k<2;k++)bird(new T.Vector3(nr+(r()-.5)*(nh.W-1.5),nh.wallH+nh.rise+TOP+.2,0),"N",{face:0,x:0,z:0});});
      spinners();
      var span=nR.ox+nR.W/2-(nL.ox-nL.W/2), S2=span/2+6; var sc2=sun.shadow.camera; sc2.left=-S2;sc2.right=S2;sc2.top=S2;sc2.bottom=-S2;sc2.near=1;sc2.far=120;sc2.updateProjectionMatrix();
      camDist=Math.max(me.W,me.Dp+2,(me.wallH+me.rise)*1.4)*1.75;
      built=key(); mt=h3.after?1:0; ms=h3.stage;
    }
    var ORDER=[[1,0],[-1,0],[1,-.32],[-1,.32],[1,.32],[-1,-.32],[1,-.44],[-1,.44],[1,.16],[-1,-.16]];
    /* spinners mount on the roof vents with two hose clamps */
    function spinners(){
      spinGroup.forEach(function(g){g.parent.remove(g);}); spinGroup=[]; ventGroup.forEach(function(g){g.parent.remove(g);}); ventGroup=[];
      var nv=Math.max(6,h3.spin);
      for(var i=0;i<nv;i++){var o=ORDER[i],face=o[0]>0?me.F:me.B,x=o[1]*me.W,z=-me.L/2+.6;
        var v=new T.Group();v.position.set(x,me.TT,z);var fl=new T.Mesh(G.flash,M.flash);fl.position.y=.01;v.add(fl);var vp=new T.Mesh(G.vent,M.vent);vp.position.y=.18;vp.castShadow=true;v.add(vp);face.add(v);ventGroup.push(v);
        if(i>=h3.spin)continue;
        var g=new T.Group();g.position.set(x,me.TT,z);
    ...
    function paintCover(){
      [1,-1].forEach(function(sign){
        ...
        var m=covM[sign]; m.scale.set(me.W,me.L,1); m.position.set(0,me.TT+.02,0);
        ...
        spinGroup.forEach(function(sp){var u=sp.userData;if(u.face!==sign)return;
          var cx=(u.x/me.W+.5)*cw, cy=(u.z/me.L+.5)*ch, rx=me.L*1.25/me.W*cw, ry=1.25*ch;
    ...
    function covered(b){
      for(var i=0;i<spinGroup.length;i++){var s=spinGroup[i].userData;
        ...
        if(dz>-.7&&Math.hypot(dx,dz)<me.L*1.25&&(dz<=0||Math.atan2(Math.abs(dx),dz)<HALF))return true;}
      return false;
    }
    function state(){
      var sv=h3.svc, m=$("ovMsg"); m.hidden=false; m.className="ov-msg";
      if(spinGroup.length!==h3.spin)spinners();
      var stg=sv==="pig"?h3.stage:0;
      spinGroup.forEach(function(g){g.visible=sv==="pig"&&stg===2;}); [1,-1].forEach(function(k){if(covM[k])covM[k].visible=sv==="pig"&&stg===2&&h3.cov;});
      var mine={1:0,"-1":0,0:0}, next=0;
      P.birds.forEach(function(b){
        if(b.kind==="N"){b.kt=0;return;}
        b.kt=stg>=1&&b.kind==="U"?1:stg===2&&covered(b)?1:0;
        if(b.kt)next++;else if(b.kind!=="U")mine[b.face]++;else mine[1]++;});
      var nbTotal=P.birds.filter(function(b){return b.kind==="N";}).length+next;
      if(sv==="pig"){
        var left=mine[1]+mine[-1]+mine[0], sf=0,sb=0; spinGroup.forEach(function(g){if(g.userData.face>0)sf++;else sb++;});
        if(stg===0)m.innerHTML="<b>Today:</b> every roof on the block has pigeons. "+P.birds.filter(function(b){return b.kind!=="N";}).length+" on yours, nesting under the panels and resting on the ridge.";
        else if(stg===1){m.className="ov-msg warn";m.innerHTML="<b>Cleanout and mesh.</b> Nests out, panels cleaned, mesh clipped on. Nothing gets under your panels again, but "+left+" pigeons still land on your open roof. That's the job of the free spinners.";}
        else if(!left)m.innerHTML="<b>Your roof is clear.</b> The free spinners keep them from coming back after the cleanout, so they settle on the unprotected roofs next door. "+nbTotal+" pigeons next door now. The first house on the block to protect wins.";
        else{m.className="ov-msg warn";
          if(!h3.spin)m.innerHTML="<b>Mesh only.</b> "+left+" pigeons still sit on your open roof. Add spinners.";
          else if(!sb&&mine[-1])m.innerHTML="<b>"+h3.spin+(h3.spin>1?" spinners only watch":" spinner only watches")+" the front.</b> The ridge blocks the flash, so "+mine[-1]+" pigeons stay on your back side. Add one facing the back.";
          else m.innerHTML="<b>"+left+" pigeon"+(left>1?"s":"")+" still outside the flash</b> at the corners. Add another spinner.";}
        ...
        return;}
      var aft=!!h3.after;
      if(sv==="win"){var one=st.stories===1,cnt=P.wins.length,sum=(one?149:249)+(st.more?(one?39:59):0)+(st.inside?49:0);
        m.innerHTML=aft?"<b>Crystal clear.</b> Outside glass, screens, tracks and sills, finished with purified water so it dries spot free.":"<b>Tap After</b> to see "+cnt+" windows finished the way we leave them.";
        $("osum").innerHTML="<b>"+(one?"1":"2")+" story · "+cnt+" windows · "+money(sum)+"</b><br>"+(one?"1 story price covers 10 to 12 windows, more is +$39.":"2 story price covers 18 to 20 windows, more is +$59.")+" Inside is +$49.";}
      else if(sv==="sol"){var n2=st.panels;
        m.innerHTML=aft?"<b>Full output.</b> Purified water and a soft brush leave every cell clear. Nobody walks on your panels.":"<b>Tap After</b> to see "+n2+" panels finished.";
        $("osum").innerHTML="<b>"+n2+" panels · "+(st.pig?"included with pigeon proofing":money(n2*7))+"</b><br>$7 a panel. Split them into arrays above to match your roof.";}
      else{var ns=Math.min(st.screens,P.wins.length),each=st.pet?64.99:53.99;
        m.innerHTML=aft?"<b>New mesh on "+ns+" screen"+(ns>1?"s":"")+".</b> Re-meshed on site, same visit, fitted to every frame.":"<b>Tap After</b> to see "+ns+" new screens fitted.";
        $("osum").innerHTML="<b>"+ns+" screens · "+money(Math.max(ns*each,149))+"</b><br>"+(st.pet?"Pet and all weather":"Standard")+" mesh at "+money(each)+" a screen. $149 job minimum when screens are the only service.";}
    }
    /* camera */
    var tiltTo=null;
    function aim(){var low=h3.svc==="win"||h3.svc==="scr";tiltTo=low?.16:.34;target.set(0,low?me.wallH*.5+.2:(me.wallH+me.rise)*.5,0);zoom=low?.62:h3.svc==="pig"?1.25:1;spinOn=true;yawTo=Math.round((yaw-.45)/(Math.PI*2))*Math.PI*2+.45;}
    var yaw=.55, tilt=.36, zoom=1, spinOn=true, yawTo=null, pts={}, pinch=0, el=R.domElement;
    el.addEventListener("pointerdown",function(e){pts[e.pointerId]=[e.clientX,e.clientY];spinOn=false;yawTo=null;tiltTo=null;try{el.setPointerCapture(e.pointerId);}catch(x){}});
    el.addEventListener("pointermove",function(e){var p=pts[e.pointerId];if(!p)return;var ids=Object.keys(pts);
      if(ids.length===1){yaw-=(e.clientX-p[0])*.008;tilt=Math.max(.08,Math.min(1.2,tilt+(e.clientY-p[1])*.005));}
      pts[e.pointerId]=[e.clientX,e.clientY];
      if(ids.length===2){var a=pts[ids[0]],b=pts[ids[1]],dd=Math.hypot(a[0]-b[0],a[1]-b[1]);if(pinch)zoom=Math.max(.4,Math.min(2.2,zoom*pinch/dd));pinch=dd;}});
    function up(e){delete pts[e.pointerId];pinch=0;}
    el.addEventListener("pointerup",up); el.addEventListener("pointercancel",up);
    el.addEventListener("wheel",function(e){e.preventDefault();zoom=Math.max(.4,Math.min(2.2,zoom*(1+e.deltaY*.001)));},{passive:false});
    ...
    var raf=0,last=0, vtmp=new T.Vector3();
    function loop(now){
      ...
      if(inHero){demoT+=dt;if(demoT>3){demoT=0;
        if(h3.svc==="pig"){h3.stage=(h3.stage+1)%3;if(h3.stage===0&&CYCLE){cycleI=1;h3.svc="win";h3.after=0;aim();}}
        else{h3.after=h3.after?0:1;if(!h3.after&&CYCLE){cycleI=(cycleI+1)%4;h3.svc=SVC[cycleI];h3.stage=0;aim();}}
        state();$("wlbl").textContent="Live 3D · "+{pig:"Pigeons",win:"Windows",sol:"Solar",scr:"Screens"}[h3.svc];}}
      if(yawTo!==null){yaw+=(yawTo-yaw)*Math.min(1,dt*2.5);if(Math.abs(yawTo-yaw)<.01)yawTo=null;}
      else if(spinOn&&!reduce&&(ov.hidden||h3.svc==="pig"))yaw+=dt*.12;
      if(tiltTo!==null){tilt+=(tiltTo-tilt)*Math.min(1,dt*2.5);if(Math.abs(tiltTo-tilt)<.005)tiltTo=null;}
      var sv=h3.svc, pg=sv==="pig", stg=pg?h3.stage:0, sT=stg>=1?1:0;
      if(ms!==sT){ms+=(sT-ms)*Math.min(1,dt*2.4);if(Math.abs(ms-sT)<.003)ms=sT;}
      var mT=h3.after?1:0; if(mt!==mT){mt+=(mT-mt)*Math.min(1,dt*2.4);if(Math.abs(mt-mT)<.003)mt=mT;}
      P.skirt.forEach(function(s){s.visible=pg&&ms>.02;s.scale.y=H*Math.min(1,ms*1.4);s.position.y=s.userData.y0+s.scale.y/2;});
      P.clips.forEach(function(c){c.visible=pg&&ms>.75;});
      /* windows and solar: dull to bright, with a shine passing across when finished */
      var gw=sv==="win"&&mt>.5; P.wins.forEach(function(w,i){w.gl.material=gw?M.glass:M.glassDull; w.sf.visible=sv==="scr"&&i<Math.min(st.screens,P.wins.length)&&mt>.5;});
      M.cell.roughness=sv==="sol"?.1+.45*(1-mt):.1; M.cell.color.setScalar(sv==="sol"?.72+.28*mt:1);
      shineT=(shineT+dt*.8)%2.2; var showSh=(sv==="win"||sv==="sol")&&mt>.9;
      M.shine.opacity=showSh?.35*Math.max(0,Math.sin(Math.min(1,shineT)*Math.PI)):0;
      P.shine.forEach(function(s){var on=showSh&&(s.panel?sv==="sol":sv==="win");s.m.visible=on;if(on)s.m.position.x=(s.panel?s.m.userData.x0:0)+(Math.min(1,shineT)-.5)*s.w;});
      /* pigeons fly from your roof to the roofs next door */
      P.birds.forEach(function(b){b.m.visible=pg;if(!pg)return;if(b.k!==b.kt){b.k+=(b.kt-b.k)*Math.min(1,dt*2.2);if(Math.abs(b.k-b.kt)<.004)b.k=b.kt;}
        if(b.dest){vtmp.copy(b.base).lerp(b.dest,b.k);vtmp.y+=Math.sin(b.k*Math.PI)*4;b.m.position.copy(vtmp);if(b.k>0&&b.k<1)b.m.rotation.y=Math.atan2(b.dest.x-b.base.x,b.dest.z-b.base.z)*(b.kt?1:-1);else b.m.rotation.y=b.ry;}});
      spinGroup.forEach(function(g,i){g.userData.rot.rotation.y+=dt*(3.2+i*.3);});
      var d=camDist*(inHero?(sv==="pig"?1.5:sv==="win"||sv==="scr"?.85:1.3):zoom)*(cam.aspect<1.1?1.3:1);
    ...
    function key(){return st.panels+"-"+Math.min(h3.arrays,st.panels)+"-"+h3.roof+"-"+h3.color+"-"+h3.stories+"-"+st.more;}
    return {
      attach:function(el2){host=el2;el2.insertBefore(R.domElement,el2.firstChild);size();},
      rebuild:function(){build();P.shine.forEach(function(s){s.m.userData.x0=s.m.position.x;});aim();state();size();},
      panels:function(){if(key()!==built){build();P.shine.forEach(function(s){s.m.userData.x0=s.m.position.x;});}state();},
      aim:aim, state:state,
