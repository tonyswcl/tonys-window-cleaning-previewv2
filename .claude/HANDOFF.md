...
- **Plans by job.** `winPlan` (outside), `roomPlan` (inside), `scrPlan` (re-mesh at a table), `solPlan` (pole wash, side on, both hands on the pole, back section crossed beside the panels). `steps()` returns the current plan's marks.
- **Test API** (`window.__tq.api`): `seek(t)`, `settle()`, `peek(shot)`, `techAt()`, `state()` (mode, style, steps, rig), `work()` (the farthest a hand got from its target, and glass, screen, track, dust, spline and mesh coverage), `tuneGrip`, `quality()` (includes `progs`, the shader count), `look({style,hood,stories,grids,roof,rc})`, `pref(mode)`, `prop(com)`.
