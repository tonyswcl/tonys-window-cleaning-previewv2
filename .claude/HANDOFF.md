...
000. Final production task, September 26, on the branch, **not merged**. Tony's look (one skin tone for face and hands, softer lips, dark brows, glossy eyes, short hair painted on instead of the speckled hair cards, a cap that sits like a cap, thin frames, a calmer blue, matte skin; `tools/tech/`), the page 3D paced evenly at 30 frames a second. Real work photos between the 3D and the quote on 76 pages, from one data file, own town first; the main gallery shows only the best photos and the real before and after. Inside is a flat $49 (it already was; now tested). Meta hears only intent events, phone and text taps now reach it as Contact. The Meta campaign package: https://claude.ai/artifact/Qt1117yJHNApzU7gi7dkEL (launch after "move to main"). Follow up the same day, from Tony: the Oak Hills solar photo (`oak-hills-solar-04`) showed old droppings on the tiles under the panel edge, left by another company, so it and `hub-solar-03` (stained frame edge) are `skip` with a `why`, and the clean Phelan array took its place. The photo viewer now opens in the middle of the screen (style.css zeroes every margin, which had pinned the dialog top left), a strip with fewer than 6 photos fills its desktop row, photo grids of 3, 4 or 5 never leave a half empty last row, the main gallery holds 6 a service, the gallery's search data lists only photos it shows, and the solar hub page's before and after is the real pair.
00. Final audit, September 26, on the branch, **not merged** (report: the audit artifact; commits "Final audit, batch 1" to "batch 6"). Property type follows the customer's choice everywhere, one shared panels and sections control, stale plans and spinners cleared, Back closes the 3D, a refresh keeps the quote. Add-ons that fit each service, honest savings. The window, inside, screen re-mesh and solar jobs are real sequences where dirt only clears where a tool passes, and the pane ends clean. Tony's body runs on one shared posture, grip and feet system. Two more homes (older desert home, manufactured home), an auto shop storefront, clearer style names. Idle start, a smaller character file, `<main>` on every page, related links, the PDF picture shows back sections. See "The work system" below.
...
The 24 pages in the masterplan artifact (https://claude.ai/artifact/HD7hioXQZVXvkx61rjRDkq) shipped in round two and have been live since September 25. Re-export Search Console in late October and adjust titles, descriptions and internal links from what it shows. The Meta plan waits on Tony ("I'll get back to you on the ad").
...
- **The character (round five, September 26).** Rebuilt from the round four file by `tools/tech/` (see its README): 562 KB, no hair cards. `buildRig()` sets matte skin, glossy eyes and dark lashes; `headwear()` fits the cap lower at the back with a slight tilt (`HW.ct`) and thin frames.
...
### The work system (final audit)

All in `_quote/src/q3d.js`. The demos are timed plans, not hand keyed poses, so they keep working when the window size, the home style, the solar layout or the camera changes.

- **Surfaces and dirt.** `surf()` is a flat patch on any object (a pane, a sill, a track, a panel, a screen). `layer()` puts a canvas dirt layer on it; tools paint it clean (`lyPoly`, `lyOp`) only where they pass, and `lyLeft()` says how much dirt is left. A pane has to end under 0.1% dirty.
- **Plans.** A plan is a list of `Clip`s on one clock (`Build()`: `at`, `add`, `place`, `mark`). Each clip gives the body (`body(k,t)`), the hands (`hands(k,t)`: `grip`, `palm`, `rest`, a carried tool), where he looks, and what gets painted. `mark()` sets the step the note and the Next button stop at. `runPlan` plays it; `seek(t)` jumps anywhere.
- **Shared jobs.** `glassJob` (wet, squeegee, towel for any number of panes), `ScreenPath` (a screen carried, turned and set down without passing through him), `toolRun` and `palmRun` (a tool or a flat hand along a path on a surface), `toolHop` and `palmHop` (moving between them), `holsterPose` and `tuckTowel`.
- **The body.** `posture()` bends at the hips or squats by target height and steps him in when a target is out of reach (`pose`, `stanceUp`). `grip()` is a two bone arm with the hand closed on the handle; `palmOn()` lays a flat hand or fingertips on a surface; `shrug()` lifts the shoulder for high reach. The feet planner keeps feet planted and steps them, on the ground or on the roof (`roofGround()`). Scenes name hands by screen side; `rigArm(side)` maps that to the rig.
- **Smooth body.** `bodyAt()` samples the plan's body 13 times over about 0.4 s with a bell shaped weight, so a stance change (reaching deep versus stepping in, a clip boundary) glides over a few frames instead of flipping between frames. Test hooks for motion: `api.trace(on)` (head, hands, feet, hips each frame), `api.bodyRaw(t0,t1,step)`, `window.__tqFixedDt` (fixed frame step) and `window.__tqNoHold` (play through step marks).
- **Plans by job.** `winPlan` (outside), `roomPlan` (inside), `scrPlan` (re-mesh at a table), `solPlan` (pole wash, side on, back section crossed beside the panels). In `solPlan` the pole is laid through the rear hand where it actually closes, and the front hand takes it where it crosses his chest (`pc`; a hand's `done(got)` and a function for the other hand make that order possible), so neither hand leaves the pole. `steps()` returns the current plan's marks.
- **Layout.** A short window (a phone on its side, height 500 or less and width 600 or more) puts the controls in a side column and the tabs in the top row (`quote.css`, the last block).
- **Test API** (`window.__tq.api`): `seek(t)`, `settle()`, `peek(shot)`, `techAt()`, `state()` (mode, style, steps, rig), `work()` (the farthest a hand got from its target with which hand, and glass, screen, track, dust, spline and mesh coverage), `tuneGrip`, `quality()` (includes `progs`, the shader count), `look({style,hood,stories,grids,roof,rc})`, `pref(mode)`, `prop(com)`.

### Real work photos

- `_quote/src/photos.json` says what each job photo shows: service (`win`, `sol`, `pig`, `scr`), the town it was taken in (null for the `hub-` photos, whose town isn't known), a tier and captions and alt text in English and Spanish. Tiers: `best` (main gallery quality), `job` (local proof), `before` (pairs and ads only), `skip` (never shown). Captions only name Tony on the two photos whose file name says it's him.
- `build.py` puts a strip of up to 6 photos (`work_block`) between the hero and the quote on every page with relevant work: the page's service (a mix of windows, solar and pigeons on general pages), the page's own town first, then the best from other towns, never a photo the page already shows elsewhere. The heading names the town only when 2 or more photos are from it; otherwise it says honestly that these are from other towns. Storefront, graffiti and decal pages get none (no photos of that work yet). The strip loads small webp thumbnails from `assets/photos/t/` (`python3 tools/pages/thumbs.py` after adding photos); a tap opens the full photo in a dialog (`workBox()` in quote.js).
- The main gallery (`gallery.html`) marks its grids with empty `<!-- tq:best-win -->` style blocks; the build fills them with the `best` photos of that service, the before and after pair first (`pairs` in the data).
- Adding photos: drop the jpg in `assets/photos/` named `town-service-nn.jpg`, add an entry to `photos.json` after looking at it, run `thumbs.py`, then `build.py`, then `tools/tests/gallery.py`.
- Look at the edges of every photo before it goes above `skip`, not just the glass: droppings, nests, stains or trash anywhere in frame read as our mess, even when another company left them (Tony, September 26). Put the reason in `why` so nobody promotes it later. `gallery.py` fails if a `skip` photo shows up on any page, hand sections and search data included.
- Rows fill: the strip carries `data-n` and quote.css widens 3, 4 or 5 tiles to fill the desktop row; `.photo-grid` in style.css lays out 3, 4 and 5 photos so the last row is full; `best_block` stops at 6 (`BEST_N`). `mobile.js` checks every grid and strip on its pages at every size.

### Meta Pixel

Pixel `1424363186269397` is in every page head (PageView). `META_MAP` in quote.js sends only: ViewContent (3D opened), CustomizeSimulator (service picked or added, home built, windows edited, style, plan or pigeon package picked), QuoteReady (PDF saved or shared), QuoteSubmitted (sent by text, email or form), Lead (form) and Contact (any phone or text link, from a listener in quote.js). Nothing else reaches Meta. Google tags from launch are untouched (GA4 `G-F97LW93P0P`, Ads `AW-17238956448`); Tony isn't running Google Ads now. The campaign plan optimizes for a custom conversion "Quote sent" on QuoteSubmitted. Ad images: `node tools/ads/render.js` (copy and layout in `tools/ads/ads.json`, output `assets/ads/meta/`).

### Property models

- Home styles `STY`: New build, Ranch, Classic, Large two story (was Lake estate), Mountain cabin, Older desert home (low hip roof, aluminum sliders), Manufactured home (skirting, a deck with steps, two row solar, shingles). `STYLE_N` and `STYLE_ES` are the names, `STYLE_D` how the message to Tony and the PDF describe the home.
- `pickStyle(v)` is the one place a style is picked, from the welcome or the panel: it sets that style's own defaults (one story, the large home flag, grids, a manufactured home's shingles) and puts back what it set when another style is picked. A roof picked by hand stays.
- Storefront styles `CSTY`: Strip center, Cafe with patio, Tall glass front, Standalone office, Auto shop (the waiting room glass plus a service wing with three roll-up bays).
...
- Every page has one `<main id="main">` between the header and the footer; `make.py` adds it to any page without one. `RELATED` in `make.py` groups pages (windows, screens, solar, pigeons, commercial, mountain towns) and each page in a group gets a row of links to the others, so no page hangs on a single link.
- The hero poster matches the page's live hero: `home-4x5.webp` (with mesh and spinners) on home and pigeon pages, `home-plain-4x5.webp` on window, solar and screen pages, `com-4x5.webp` on commercial pages (`poster_of()` in `build.py`). `cards.js` renders them (`POSTERS=1` for only the posters, `STYLES=5,6` for only some style cards).
- Start up: the 3D starts when the browser is idle; the home and street are built in a second task (`built()`); the character (`tech.glb`, meshopt compressed, 0.8 MB) loads when the page is idle and the hero is on screen, or when the 3D opens on a slow connection. `gltf.min.js` bundles the glTF loader with the meshopt decoder. The first frame still compiles the shaders in one go; three.js r128 has no way to spread that out (a staged warm up was tried and measured, and it made the total work larger).
...
| `modes3d.js` | Welcome with the count rows, Tony's intro, tour, folded panel, pigeon chips and story steps, neighborhoods, mountain cabin, manufactured and older desert homes, Ask Tony, solar chips and spinners only, the window job ends clean at its last step, inside from the picture | `no errors`, every logged check true |
| `home-flow.js 390 844` | Welcome to Fix it on a phone, damage dots, weather chip | `errors []` |
| `storefront-flow.js 1280 800` | Storefront tour, stickers, the auto shop, building mode, price lines | `errors []`, `auto shop built 4` |
...
| `models.js` | All seven home styles build, the style pick sets and restores its own defaults, the message names the home, the auto shop builds | 13 PASS |
| `gallery.py` (python, no browser) | The real work strip on every page: placement between the 3D and the quote, service fit, own town first, honest headings, captions name the right town, no repeats, files exist; the main gallery shows only best photos and at most 6 a service, no `skip` photo on any page, search data names only photos the page shows | 16 PASS |
| `inside.js` | Inside is a flat $49 on the ticket, with two stories, more windows, a 60 window house, a plan, in Spanish and in the PDF | 11 PASS |
| `meta.js` | One pixel everywhere, the two launch Google ids only, and on a real journey Meta hears only intent events (photo taps send nothing) | 13 PASS |
| `motion.js` | The window demo played frame by frame through the crouch at the wall: head and hips never jump (the crouch used to flip every other frame) | 4 PASS |
| `mobile.js` | 360, 390, 768 wide and landscape phone and tablet: no sideways scroll, 3D height, strip after the 3D and before the quote, thumbnails only, no half empty photo rows, the photo viewer centered and still centered after Next | 256 pass |
| `similarity.py`, `audit.py` | Page overlap by type, on page SEO | see numbers above |

Run the browser tests one or two at a time: SwiftShader is slow, and with three at once a check with a fixed wait can miss. `audit.py` counts decoded characters (so `&amp;` is one) and treats `alt=""` as decorative. Tests set `window.__tqFixedQ=true` so the quality watcher doesn't lower detail in screenshots. The 3D in headless Chromium runs on SwiftShader at a few frames a second, so tests call `window.__tq.api.settle()` to skip camera flights.

Preview: `PREVIEW_OUT=<scratchpad>/preview python3 tools/tests/mkpreview.py` builds `<scratchpad>/preview/site` (every page with `live:false`, `index.html` renamed `home.html`, a landing page from `tools/tests/preview-landing.html`; edit its "New this round" list by hand) and `files-v3.json` for publishing to the preview artifact above with the Artifact tool's `files` map and `root` set to that site folder. The Artifact tool only accepts a `root` inside the repo or the session scratchpad, so don't build it under `/tmp`. The artifact host refuses `.glb` files, so the preview leaves `tech.glb` out and ships the same bytes as `assets/quote/tech-glb.js` (a base64 data URI); each preview page's `TQ.techjs` names it and `loadRig()` loads it before the character. The live site never sets `techjs`. An update only needs the changed files in `files`: publishing all 187 again is slow.
