...
| `gallery.py` (python, no browser) | The real work strip on every page: placement between the 3D and the quote, service fit, own town first, honest headings, captions name the right town, no repeats, files exist; the main gallery shows only best photos | all PASS |
...
| `mobile.js` | 360, 390, 768 wide and landscape phone and tablet: no sideways scroll, 3D height, strip after the 3D and before the quote, thumbnails only | 156 pass |
