...
const styles=[[0,1,false,{yaw:.62,tilt:.22,dist:.95}],[1,1,false,{yaw:.62,tilt:.22,dist:.95}],[2,1,true,{yaw:.62,tilt:.22,dist:.95}],[3,2,false,{yaw:.62,tilt:.2,dist:.95}]];
...
for(const h of [0,1,2,3]){await set({look:{style:0,hood:h,stories:1,grids:false}});save('hood-'+h,await p.evaluate(c=>window.__tq.api.snap(480,320,c),{yaw:h===3?.35:.55,tilt:.42,dist:h===2?2.4:2.1}));}
...
for(const [n,w,h,o] of [['home-4x5',800,1000,{yaw:.6,tilt:.3,dist:1.0}],['home-16x10',960,600,{yaw:.62,tilt:.2,dist:1.05}]])save(n,await p.evaluate(a=>window.__tq.api.snap(a[0],a[1],a[2]),[w,h,o]));
