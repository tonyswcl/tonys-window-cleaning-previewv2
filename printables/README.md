...
Handed to every customer at the end of a job. `goodiebag-sheet.html` →
`pdf/goodiebag-sheet.pdf` and `goodiebag/tonys-goodie-bag-spec.jpg` is the
one-page package and order spec.

| Piece | Trim | Qty per 100 bags |
|---|---|---|
| `card-thankyou-front` / `-back` | 5 × 7" | 100 double-sided |
| `card-referral-front` / `-back` | 3.5 × 2.5" | 250 double-sided |
| `coozie-wrap` | 4 × 4" per panel | 100 |
| `sticker-bag-seal` | 2.5" round | 150 |

QR codes live in `assets/qr/` and are generated by `build-qr.py` from the
links the site publishes — Google review, Facebook page, Yelp page. All
four were decode-verified after rendering.
...
solicitation outright, which is why its code is labelled "find us" rather
than "review us". The thank-you card asks for an honest review with
nothing attached — that is allowed — and the money sits on the referral
card, where paying for a customer introduction is perfectly ordinary.
