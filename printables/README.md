...
## Client goodie bag & field kit

`goodiebag-sheet.html` -> `pdf/goodiebag-sheet.pdf` and
`goodiebag/tonys-goodie-bag-spec.jpg` is the one-page package and order
spec. Every size below is a UPrinting stock product, so nothing needs a
custom-size upcharge.

| Piece | Trim | UPrinting product | Qty per 100 bags |
|---|---|---|---|
| `card-thankyou-front` / `-back` | 5 x 7" | Postcards | 100 double-sided |
| `card-referral-front` / `-back` | 3.5 x 2" | Business Cards | 250 double-sided |
| `magnet-fridge-3.5x2` | 3.5 x 2" | Business Card Magnets | 100 |
| `sticker-bag-seal` | 2.5" round | Circle Stickers | 150 |
| `doorhanger-4.25x11` | 4.25 x 11" | Door Hangers | 500 |
| `yardsign-18x24` | 18 x 24" | Yard Signs | 2 |
| `coozie-wrap` | 4 x 4" per panel | not UPrinting - 4imprint | 100 |

Flat JPGs for phone upload live in `upload/`. QR codes are generated by
`build-qr.py` from the links the site publishes, and every one was
decode-verified out of the finished print file.

**No solid color fields.** Navy is used as ink on a light ground
throughout - a filled navy panel reads as a sticker rather than as
stationery, the same reason the vehicle graphics dropped their plates.

**The reward is attached to referrals, not reviews.** Google's policies
prohibit incentivized reviews and filter or remove them; Yelp penalizes
solicitation outright, which is why its code reads "find us" rather than
"review us". The thank-you card asks for an honest review with nothing
attached, and the $50 sits on the referral card where paying for a
customer introduction is perfectly ordinary.
